...
const TaskCard = ({ task, index, handleTaskChanged }) => {
  ...
  const updateTask = async () => {
    try {
      ...
      console.error("Lỗi xảy ra khi chỉnh sửa nhiệm vụ", error);
      toast.error("Lỗi xảy ra khi chỉnh sửa nhiệm vụ mới.");
    }
  };
        ...
        <div className="flex-1 min-w-0">
          {isEditing ? (
            <Input
              ...
              onBlur={() => {
                setIsEditing(false);
                setUpdateTaskTitle(task.title || ""); //reset lại tiêu đề ban đầu
              }}
            />
          ) : (
            <p
