  ...
  const addTask = async () => {
    if (newTaskTitle.trim()) {
      try {
        ...
        handleNewTaskAdded(); // thông báo đã tạo nhiệm vụ mới cho component cha, render lại các component khác.
      } catch (error) {
        console.error("Lỗi xảy ra khi thêm nhiệm vụ", error);
        toast.error("Lỗi xảy ra khi thêm nhiệm vụ mới.");
      }
      setNewTaskTitle(""); //xóa nội dung trong ô input sau khi thêm nhiệm vụ
    } else {
    ...
    <Card className="p-6 border-0 bg-gradient-card shadow-custom-lg">
      <div className="flex flex-col gap-3 sm:flex-row">
        <Input
          ...
          onKeyPress={handleKeyPress} // giống "thêm" nhưng là nút "Enter" trên bàn phím
        />
        <Button
          variant="gradient"
          size="xl"
